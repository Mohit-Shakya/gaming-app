          ...
          {activeTab === 'subscriptions' && (
            <div style={{ display: "flex", flexDirection: "column", gap: isMobile ? 16 : 24 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: isMobile ? 12 : 16 }}>
                <div>
                  <h2 style={{ fontSize: isMobile ? 20 : 24, fontWeight: 700, color: theme.textPrimary, margin: 0 }}>
                    Subscriptions
                  </h2>
                  <p style={{ fontSize: isMobile ? 12 : 14, color: theme.textMuted, margin: "4px 0 0 0" }}>
              ...
              {/* Tabs: Subscriptions & Plans */}
              <div style={{ display: "flex", gap: 12, borderBottom: `1px solid ${theme.border}`, paddingBottom: 0 }}>
                <button
                  style={{
                    padding: "12px 20px",
                    background: "transparent",
              ...
              {/* Search and Filters */}
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr auto auto", gap: 12 }}>
                ...
                >
                  ...
                  {membershipPlans.filter(p => p.type === 'hourly_package').map(plan => (
              ...
              <div style={{
                ...
                <div style={{
                  display: "grid",
                  gridTemplateColumns: isMobile ? "1fr" : "2fr 1.5fr 1fr 1fr 1fr 1fr auto",
                  gap: 16,
                  padding: "16px 20px",
                  background: "rgba(59, 130, 246, 0.05)",
                  borderBottom: `1px solid ${theme.border}`,
                  ...
                  <div>CUSTOMER</div>
                  {!isMobile && <div>PLAN</div>}
                  {!isMobile && <div>TIME REMAINING</div>}
                  {!isMobile && <div>EXPIRES</div>}
                  {!isMobile && <div>PAID</div>}
                ...
                {(() => {
                  const filteredSubscriptions = subscriptions.filter(sub => {
                    ...
                    if (subscriptionPlanFilter !== 'all' && sub.membership_plan_id !== subscriptionPlanFilter) return false;
                    if (subscriptionSearchQuery) {
                      const query = subscriptionSearchQuery.toLowerCase();
                      return sub.customer_name.toLowerCase().includes(query) ||
                  ...
                  if (filteredSubscriptions.length === 0) {
                    return (
                      <div style={{ padding: "60px 20px", textAlign: "center" }}>
                        <div style={{ fontSize: 48, marginBottom: 16, opacity: 0.3 }}>📋</div>
                        <p style={{ fontSize: 16, color: theme.textSecondary, fontWeight: 500 }}>
                          No subscriptions found
                        </p>
                        <p style={{ fontSize: 14, color: theme.textMuted }}>
                          {subscriptionSearchQuery || subscriptionStatusFilter !== 'all' || subscriptionPlanFilter !== 'all'
                  ...
                  return filteredSubscriptions.map((sub, index) => {
                    ...
                    const timeRemaining = `${sub.hours_remaining}h ${sub.hours_purchased > 0 ? `/ ${sub.hours_purchased}h` : ''}`;
                    const expiryDate = new Date(sub.expiry_date);
                    const isActive = sub.status === 'active';
                    const progress = (sub.hours_remaining / sub.hours_purchased) * 100;
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          gap: 16,
                          padding: "20px",
                          borderBottom: index < filteredSubscriptions.length - 1 ? `1px solid ${theme.border}` : 'none',
                          alignItems: "center",
                        }}
                      >
                        {/* Customer */}
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                          <div style={{
                            ...
                            {sub.customer_name.charAt(0).toUpperCase()}
                          </div>
                          <div>
                            <div style={{ fontSize: 14, fontWeight: 600, color: theme.textPrimary }}>
                              {sub.customer_name}
                          ...
                          <div>
                            <div style={{
                              ...
                              <div style={{
                                width: `${progress}%`,
                                height: "100%",
                                background: progress > 50 ? "#10b981" : progress > 20 ? "#f59e0b" : "#ef4444",
              ...
              {/* Pagination Info */}
              <div style={{ fontSize: 13, color: theme.textMuted, textAlign: "center", padding: "12px 0" }}>
                Showing 1 to {subscriptions.length} of {subscriptions.length} subscriptions
              </div>
            </div>
          )}
      ...
      {showAddSubscriptionModal && (
        ...
        >
          ...
          >
            ...
            {/* Form */}
            <div style={{ padding: isMobile ? "20px 24px" : "32px", display: "flex", flexDirection: "column", gap: isMobile ? 16 : 20 }}>
              {/* Customer Name */}
              <div>
                <label style={{ display: "block", fontSize: isMobile ? 12 : 13, fontWeight: 600, color: theme.textSecondary, marginBottom: 8 }}>
                  Customer Name *
                </label>
                <input
                  type="text"
                ...
                >
                  ...
                  {membershipPlans.filter(p => p.type === 'hourly_package').map(plan => (
                    <option key={plan.id} value={plan.id}>
                      {plan.name} - {plan.hours}h valid for {plan.validity_days} days (₹{plan.price})
                    </option>
                  ))}
                </select>
                <p style={{ fontSize: isMobile ? 10 : 11, color: theme.textMuted, marginTop: 6, marginBottom: 0 }}>
                  Only hourly packages can be used for subscriptions
                </p>
              </div>
              ...
              <div>
                ...
                />
                <p style={{ fontSize: isMobile ? 10 : 11, color: theme.textMuted, marginTop: 6, marginBottom: 0 }}>
                  {newSubPlanId && membershipPlans.find(p => p.id === newSubPlanId)
                    ? `Plan price: ₹${membershipPlans.find(p => p.id === newSubPlanId).price}`
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                ...
                <button
                  onClick={async () => {
                    if (!newSubCustomerName || !newSubCustomerPhone || !newSubPlanId || !newSubAmountPaid) {
                    ...
                    const subscriptionData = {
                      cafe_id: cafes[0].id,
                      customer_name: newSubCustomerName,
                      customer_phone: newSubCustomerPhone,
                      customer_email: newSubCustomerEmail || null,
                      membership_plan_id: newSubPlanId,
                      hours_purchased: selectedPlan.hours,
                      hours_remaining: selectedPlan.hours,
